...
cd "$cache/gdb-build"
"$cache/gdb-17.1/configure" --disable-binutils --disable-ld --disable-gas --disable-gprof --disable-gprofng --disable-sim --disable-nls --disable-werror --without-python --without-guile --without-debuginfod --with-expat=yes --with-lzma=no --with-babeltrace=no --disable-tui --with-system-readline=no LDFLAGS='-static-libstdc++ -static-libgcc'
...
strip "$stage/bin/gdb"
for library in libtinfo.so.6 libmpfr.so.6 libgmp.so.10 libexpat.so.1; do
...
for package in libtinfo6 libmpfr6 libgmp10 libexpat1; do

...
//     for array types.
//
// "PreferredVectorElementSize"
//     for some types (e.g.: SIMD vectors), used to determine how large each element in the vector is when doing
//     operations like displaying the vector.
// 
// "ActionName"
...
enum PreferredFormat
{
    ...
    // The preferred format (if applicable) is integer.  Generally applied to things like SIMD vectors
    FormatInteger,

    // The preferred format (if applicable) is floating point.  Generally applied to things like SIMD vectors
    FormatFloat,
};
